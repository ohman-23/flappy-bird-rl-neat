...
from flappy_neat_rl.sprites import Button, Bird, Pipe
...
class Game(ABC):
    ...
    @abstractmethod
    def detect_and_handle_collisons(self):
        pass

    # LOG CURRENTLY LEAVING OFF: REMOVE AUTO RESET OF GAME, THAT WILL BE HANDLED BY OUTSIDE CALLING SCRIPT
    # GAME WILL NOT AUTO RESET
    def run(self):
        pygame.init()
        ...
        while run:
            ...
            self.draw_text(screen, str(self.game_instance.score), font, white, int(Config.SCREEN_WIDTH/2), 20)
            ...
            if not self.game_instance.game_over and self.game_instance.game_started:
                ...
                self.game_instance.ground_scroll = 0 if abs(self.game_instance.ground_scroll) > 35 else (GameInstance.ground_scroll - Config.SCROLL_SPEED)
